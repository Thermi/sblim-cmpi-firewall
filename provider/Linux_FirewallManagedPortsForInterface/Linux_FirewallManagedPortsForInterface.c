...
#ifndef CMPI_VER_100
#define Linux_FirewallManagedPortsForInterface_ModifyInstance Linux_FirewallManagedPortsForInterface_SetInstance
#endif

/// ----------------------------------------------------------------------------
/// ECLIPSE GLOBAL VARIABLES
/// ----------------------------------------------------------------------------
...
/// ----------------------------------------------------------------------------
CMPIStatus Linux_FirewallManagedPortsForInterface_EnumInstanceNames(
	...
	const CMPIObjectPath * reference) 	/** [in] Contains target namespace and classname. */
{
    ...
    ra_status = Linux_FirewallManagedPortsForInterface_getResources( &resource, decide_flag);
	...
	ra_status = Linux_FirewallManagedPortsForInterface_setInstanceFromResource(&resource, instance, _BROKER, decide_flag, lnamespace, context);
	if (ra_status.rc != RA_RC_OK) {
    ...
    resource = temp;
    ra_status = Linux_FirewallManagedPortsForInterface_freeResource( resource );
    if ( ra_status.rc != RA_RC_OK ) {
...
/// ----------------------------------------------------------------------------
CMPIStatus Linux_FirewallManagedPortsForInterface_EnumInstances(
	CMPIInstanceMI * self,			/** [in] Handle to this provider (i.e. 'self'). */
	const CMPIContext * context,		/** [in] Additional context info, if any. */
	const CMPIResult * results,		/** [out] Results of this operation. */
	const CMPIObjectPath * reference,	/** [in] Contains target namespace and classname. */
...
/// ----------------------------------------------------------------------------
CMPIStatus Linux_FirewallManagedPortsForInterface_GetInstance(
	CMPIInstanceMI * self,			/** [in] Handle to this provider (i.e. 'self'). */
	const CMPIContext * context,		/** [in] Additional context info, if any. */
	const CMPIResult * results,		/** [out] Results of this operation. */
	...
	const char ** properties)		/** [in] List of desired properties (NULL=all). */
{
    ...
    firewall_port4interface_t * assoc = NULL;	/** Handle to each system resource. */
    firewall_port4interface_t * resource = NULL;	/** Handle to each system resource. */
    firewall_port4interface_t * temp = NULL;	        
    //int decide_flag = 1;
    int decide_flag = 0;
    ...
    /** Get a handle to the list of system resources. */
    ra_status = Linux_FirewallManagedPortsForInterface_getResources(&assoc, decide_flag);
    if ( ra_status.rc != RA_RC_OK ) {
        build_ra_error_msg ( _BROKER, &status, CMPI_RC_ERR_FAILED, _("Failed to get list of system resources"), ra_status );
        free_ra_status(ra_status);
        goto exit;
    ...
    temp = assoc;
    ra_status = Linux_FirewallManagedPortsForInterface_getResourceForObjectPath(&assoc, &resource, reference);
    if ( ra_status.rc != RA_RC_OK) {
	build_ra_error_msg ( _BROKER, &status, CMPI_RC_ERR_FAILED, _("Failed to get resource data"), ra_status);
	goto exit;
    } else if ( !resource ) {
    ...
    assoc = temp;
    ra_status = Linux_FirewallManagedPortsForInterface_freeResource( assoc );
    if ( ra_status.rc != RA_RC_OK ) {
...
/// ----------------------------------------------------------------------------
CMPIStatus Linux_FirewallManagedPortsForInterface_ModifyInstance(
	CMPIInstanceMI * self,			/** [in] Handle to this provider (i.e. 'self'). */
	const CMPIContext * context,		/** [in] Additional context info, if any. */
	const CMPIResult * results,		/** [out] Results of this operation. */
	const CMPIObjectPath * reference,	/** [in] Contains the target namespace, classname and object path. */
...
/// ----------------------------------------------------------------------------
CMPIStatus Linux_FirewallManagedPortsForInterface_CreateInstance(
	CMPIInstanceMI * self,			/** [in] Handle to this provider (i.e. 'self'). */
	const CMPIContext * context,		/** [in] Additional context info, if any. */
	const CMPIResult * results,	 	/** [out] Results of this operation. */
	...
	const CMPIInstance * newinstance)	/** [in] Contains the new instance data. */
{
    ...
    ra_status = Linux_FirewallManagedPortsForInterface_checkForExistence(reference);
    if ( ra_status.rc != RA_RC_OK) {
    ...
    temp = resource;
    /** Set the instance property values from the resource data. */
    ra_status = Linux_FirewallManagedPortsForInterface_createResourceFromInstance( &resource, decide_flag, newinstance, _BROKER);
    if( ra_status.rc != RA_RC_OK ) {
...
/// ----------------------------------------------------------------------------
CMPIStatus Linux_FirewallManagedPortsForInterface_DeleteInstance(
	CMPIInstanceMI * self,			/** [in] Handle to this provider (i.e. 'self'). */
	const CMPIContext * context,		/** [in] Additional context info, if any. */
	const CMPIResult * results,		/** [out] Results of this operation. */
	const CMPIObjectPath * reference)	/** [in] Contains the target namespace, classname and object path. */
{
    ...
    /** Set the instance property values from the resource data. */
    ra_status = Linux_FirewallManagedPortsForInterface_deleteResource(&resource, decide_flag);
    if( ra_status.rc != RA_RC_OK ) {
...
/// ----------------------------------------------------------------------------
CMPIStatus Linux_FirewallManagedPortsForInterface_ExecQuery(
	CMPIInstanceMI * self,			/** [in] Handle to this provider (i.e. 'self'). */
	const CMPIContext * context,		/** [in] Additional context info, if any. */
	const CMPIResult * results,		/** [out] Results of this operation. */
...
/// first loaded.
/// ----------------------------------------------------------------------------
CMPIStatus Linux_FirewallManagedPortsForInterface_Initialize(
	CMPIInstanceMI * self,		/** [in] Handle to this provider (i.e. 'self'). */
	const CMPIContext * context)		/** [in] Additional context info, if any. */
{
...
/// ----------------------------------------------------------------------------
static CMPIStatus Linux_FirewallManagedPortsForInterface_Cleanup(
	CMPIInstanceMI * self,			/** [in] Handle to this provider (i.e. 'self'). */
	const CMPIContext * context,		/** [in] Additional context info, if any. */
...
/// first loaded.
/// ----------------------------------------------------------------------------
static CMPIStatus Linux_FirewallManagedPortsForInterface_AssociationInitialize(
...
/// Perform any necessary cleanup immediately before this provider is unloaded.
/// ----------------------------------------------------------------------------
static CMPIStatus Linux_FirewallManagedPortsForInterface_AssociationCleanup(
	CMPIAssociationMI * self,	/** [in] Handle to this provider (i.e. 'self'). */
	const CMPIContext * context,		/** [in] Additional context info, if any. */
	CMPIBoolean terminating)
{
...
/// AssociatorNames()
/// ----------------------------------------------------------------------------
static CMPIStatus Linux_FirewallManagedPortsForInterface_AssociatorNames(
	CMPIAssociationMI * self,	    /** [in] Handle to this provider (i.e. 'self'). */
	const CMPIContext * context,	    /** [in] Additional context info, if any. */
	const CMPIResult * results,	    /** [out] Results of this operation. */
	...
	const char * resultRole)
{
    ...
    //int decide_flag = 1;
    int decide_flag = 0;
    int typeflag = 0; /** set to 1 for Linux_FirewallInterfaces and 0 for Linux_FirewallManagedPorts */
    char InstId[15];
	...
	if(typeflag) { //object path provided is of Linux_FirewallInterface
		...
		continue;
          ...
          ra_status = Linux_FirewallManagedPortsForInterface_getObjectPathForResource( &resource, _BROKER, lnamespace, context, typeflag, results, &PrtObjPath );
          CMReturnObjectPath(results, PrtObjPath);  
          printf("PrtObjPath = %s\n", CMGetCharsPtr(CMObjectPathToString( PrtObjPath, &status),&status));
     
        }
        else { //object path provided is of Linux_FirewallManagedPorts
		 sprintf(InstId, "%d.%d.%u", resource->port.port,resource->port.end_port,resource->port.protocol);		
		 if(strcmp(srcId, InstId) ){
			resource++;
			continue;
                 }
	
          ra_status = Linux_FirewallManagedPortsForInterface_getObjectPathForResource( &resource, _BROKER, lnamespace, context, typeflag, results, &GrpObjPath );
...
/// Associators()
/// ----------------------------------------------------------------------------
static CMPIStatus Linux_FirewallManagedPortsForInterface_Associators(
	CMPIAssociationMI * self,	/** [in] Handle to this provider (i.e. 'self'). */
	const CMPIContext * context,		/** [in] Additional context info, if any. */
	const CMPIResult * results,		/** [out] Results of this operation. */
...
/// ReferenceNames()
/// ----------------------------------------------------------------------------
static CMPIStatus Linux_FirewallManagedPortsForInterface_ReferenceNames(
	CMPIAssociationMI * self,	    /** [in] Handle to this provider (i.e. 'self'). */
	const CMPIContext * context,	    /** [in] Additional context info, if any. */
	const CMPIResult * results,	    /** [out] Results of this operation. */
	const CMPIObjectPath * reference,   ///** [in] Contains the source namespace, classname and object path.
	...
	const char *role)
{
    ...
    /** Enumerate thru the list of system resources and return a CMPIInstance for each. */
    while(ra_status.rc == RA_RC_OK && resource->port.port){
	    ...
	    ra_status = Linux_FirewallManagedPortsForInterface_setInstanceFromResource(&resource, assocInst, _BROKER, decide_flag, lnamespace, context);
	    if (ra_status.rc != RA_RC_OK) {
		build_ra_error_msg( _BROKER, &status, CMPI_RC_ERR_FAILED, _("Failed to set property values from resource data"), ra_status);
		goto exit;
	    }
	    ...
	    CMReturnObjectPath(results, assocOp);
	        ...
	        sprintf(InstId, "%d.%d.%d", resource->port.port, resource->port.end_port,resource->port.protocol);
                 if(strcmp(srcId, InstId) ){
                        resource++;
                        continue;
                 }
...
/// References()
/// ----------------------------------------------------------------------------
static CMPIStatus Linux_FirewallManagedPortsForInterface_References(
	CMPIAssociationMI * self,	    /** [in] Handle to this provider (i.e. 'self'). */
	const CMPIContext * context,	    /** [in] Additional context info, if any. */
	const CMPIResult * results,	    /** [out] Results of this operation. */
...
/// ============================================================================
CMInstanceMIStub(Linux_FirewallManagedPortsForInterface_, Linux_FirewallManagedPortsForInterfaceProvider, _BROKER, Linux_FirewallManagedPortsForInterface_Initialize(&mi, ctx));
CMAssociationMIStub(Linux_FirewallManagedPortsForInterface_, Linux_FirewallManagedPortsForInterfaceProvider, _BROKER, Linux_FirewallManagedPortsForInterface_AssociationInitialize(&mi, ctx));
